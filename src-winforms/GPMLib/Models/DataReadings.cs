...
namespace GPMLib {
    ...
    public class DataReadings {
        ...
        public static List<(DisplayPowerOptionsEnum, string, string, string)> PowerDisplayOptions = new List<(DisplayPowerOptionsEnum, string, string, string)> {
        ...
        //public List<int> StatsMidPercentages = new List<int>() { 0, 25, 75, 100 };
        ...
        public List<DataSample> ReadingsAt(int x1, int x2, List<PowerTargetValueViewModel> targets) {
            ...
            lock (_LoggedReadingsLock) {
                ...
                if (x2 <= x1) { goto bail; }
                var diff = x2 - x1 - 1;
                var max = _LoggedReadings.MaxBy(o => o.Ah).FirstOrDefault();
                var min = _LoggedReadings.MinBy(o => o.Ah).FirstOrDefault();
        ...
        //private LogWriter? TheFileLogger { get; set; }
